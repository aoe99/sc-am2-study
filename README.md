# 情報処理安全確保支援士 午前II 学習ツール

情報処理安全確保支援士試験（SC）午前IIの過去問19回分（475問）を、手元のPDFから
抽出して学習アプリで使える形にするためのリポジトリ。
...
各回のフォルダに次の3ファイルがある前提（19フォルダ × 3 = 57ファイル）。

```
R07aki/R07a_2_午前II_1問題.pdf        IPA公式の問題冊子（スキャン画像・テキスト層なし）
R07aki/R07a_2_午前II_2解答例.pdf      IPA公式の解答例（テキストあり）
R07aki/R07a_2_午前II_4教科書解説.pdf  翔泳社 読者特典の解答・解説（テキストあり）
```
