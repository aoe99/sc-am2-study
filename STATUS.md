...
| 枠だけを600dpiで読み直す（2026-09-06） | **効いた。** 記号入りが589→755個。採用時の正確さ97% |
...
**本文の空欄の枠は、記号が読めているものが755個、読めていないものが893個。** 素直に
...
**枠だけを600dpiで切り出して読み直すと、記号は589個から755個になった。**
（`pdfkit-tool rectocr` ＋ `resolve_frames`）採用するのは、最大3つの読み取り候補のうち
その大問の解答例が挙げるラベルに一致するものが**ちょうど1つ**のときだけ。ページ読みで
記号が取れていた枠を正解にして較正したところ、**採用時の正確さ97%**。枠の上下の
取り方を変えて2回読んで候補を合わせると復元率が63%→70%に上がり、正確さは落ちなかった。
