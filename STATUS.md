...
| 枠だけを600dpiで読み直す（2026-09-06） | **効いた。** 記号入りが589→886個。採用時の正確さ97% |
...
**本文の空欄の枠は、記号が読めているものが886個、読めていないものが805個。** 素直に
...
**枠だけを600dpiで切り出して読み直すと、記号は589個から886個になった。**
...
正解にして較正したところ、**採用時の正確さ97%・復元率76%**。枠の上下の取り方を変えて
2回読み、同じ位置のかたまりの候補を合わせている。
...
290/427。残りは範囲表記（「［a］～［e］」の2枠に対しラベルは5つ）が大半。
