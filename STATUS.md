...
- 問題データ:
  ...
  - 午後 19回 = **90事例 / 838設問**（記号選択113 / 語句308 / 記述417）、図表750枚
...
- **問題IDを変えない。** `states` は questionId をキーにしている。午後の
  ...
  履歴が全部孤立する
- **図表を base64 でJSONに戻さない。** 午後は750枚・70MBあり、data: URI にすると
...
**設問文が取れているのは 837/838（99.9%）。** 残る1件は R05秋 問4 設問2。三つの記入例を
並べる解答例の表がテキスト層では列の順に読めず、設問2の小問と対応が付かない。
...
**本文の空欄の枠は、記号が読めているものが585個、読めていないものが990個。** 素直に
