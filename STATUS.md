...
- 問題データ:
  ...
  - 午後 19回 = **90事例 / 838設問**（記号選択113 / 語句308 / 記述417）、図表762枚
...
- **問題IDを変えない。** `states` は questionId をキーにしている。午後の
  ...
  履歴が全部孤立する
- **図表を base64 でJSONに戻さない。** 午後は762枚・72MBあり、data: URI にすると
...
| 枠だけを600dpiで読み直す（2026-09-06） | **効いた。** 記号入りが589→949個。採用時の正確さ97% |
...
**本文の空欄の枠は、記号が読めているものが949個、読めていないものが886個。** 素直に
...
**枠だけを600dpiで切り出して読み直すと、記号は589個から949個になった。**
...
6,887→2,262、図表は762枚のまま。隠す行はキャプションの向きに歩いて決め、**絵の矩形の
