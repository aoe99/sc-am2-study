...
- 問題データ:
  ...
  - 午後 19回 = **90事例 / 838設問**（記号選択113 / 語句308 / 記述417）、図表760枚
...
- **問題IDを変えない。** `states` は questionId をキーにしている。午後の
  ...
  履歴が全部孤立する
- **図表を base64 でJSONに戻さない。** 午後は760枚・70MBあり、data: URI にすると
...
| 枠だけを600dpiで読み直す（2026-09-06） | **効いた。** 記号入りが589→941個。採用時の正確さ97% |
...
**本文の空欄の枠は、記号が読めているものが941個、読めていないものが793個。** 素直に
...
**枠だけを600dpiで切り出して読み直すと、記号は589個から938個になった。**
